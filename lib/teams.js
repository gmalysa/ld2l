	...
	function(env, after, players) {
		...
		if (players.length > 0)
		{
			...
			players.forEach(function(v, k) {
				v.linear_medal = seasons.getDraftValue(v, env.team.season.linearization);
...
var setTeam = new fl.Chain(
	...
	function(env, after) {
		env.player.teamid = env.team.id;
		env.filters.signups.update({
			teamid : env.team.id,
			cost : env.player.cost,
		}, {
...
/**
 * Set the amount of money available to a team in auction when autocash is turned off
 * This expects security checks to be done beforehand
 * @param[in] team Set this team's starting money
 * @param[in] cash Set it to this amount
 */
var setStartingMoney = new fl.Chain(
	function(env, after, team, money) {
		env.money = money;
		env.team = team;
		env.filters.teams.update({starting_money : money}, {id : team.id})
			.exec(after, env.$throw);
	},
	function(env, after) {
		after(env.user, audit.EVENT_SET_STARTING_CASH, env.team, {
			starting_money : env.money
		});
	},
	audit.logTeamEvent
).use_local_env(true);
