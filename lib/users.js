...
var getSignupHistory = new fl.Chain(
	...
	function(env, after, results) {
		...
		env.filters.teams.select({id : env.teamIDs}).exec(after, env.$throw);
	},
	function(env, after, teams) {
		env.teams = {};

		_.each(teams, function(v, k) {
			env.teams[v.id] = v;
		});
	...
	new fl.LoopChain(
		...
		function(env, after) {
			after(env.teams[env.teamIDs[env.idx]].captainid);
		},
		getUser,
		function(env, after, user) {
			env.teams[env.teamIDs[env.idx]].captain = user;
			env.idx += 1;
			after();
		}
	),
