 ...
 */
function vouchUser(env, after, voucher, player) {
	if (!privs.hasPriv(voucher.privs, privs.PRIV_VOUCH)) {
		env.$throw(new Error('You do not have the ability to vouch players'));
		return;
	}
	else {
		env.filters.privs.insert({steamid : player.steamid, priv : PRIVS_JOIN_SEASON})
			.exec(after, env.$throw);
	}
}
