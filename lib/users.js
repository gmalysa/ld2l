...
/**
 * Force a given 32/64 bit ID to 32 bits
 * @param[in] id 64 or 32 bit steam ID
...
function getID32(id) {
	...
	return steamid.sub(steamOffset).toString();
}

/**
 * Force a given 32/64 bit ID to 64 bit
 * @param[in] id 32 or 64 bit steam id
 * @reutnr 64 bit version of that ID, as a string
 */
function getID64(id) {
	var steamid = new BigNumber(id+'');
	if (steamid.lessThan(steamOffset)) {
		return steamid.add(steamOffset).toString();
	}
	return steamid.toString();
}
	...
	new fl.Chain(
		...
		function(env, after, response, body) {
			...
			if (1 == data.response.success) {
				...
				env.$throw(new Error('No steamid found matching given id '+env.id));
			}
		}
	)
).use_local_env(true);
...
var getMedal = new fl.Chain(
	function(env, after, id32) {
		request('https://api.opendota.com/api/players/' + id32, env.$check(after));
	},
	function(env, after, response, body) {
		var data = JSON.parse(body);
		if (data.rank_tier)
	...
	function(env, after, user) {
		if (user !== null) {
			...
			if (env.profile !== undefined) {
				...
				env.filters.users.update(newUser, {steamid : user.steamid}).exec(after, env.$throw);
			}
			else {
				after();
			}
			...
			env.addUser$user = newUser;
			env.filters.users.insert(newUser).exec(after, env.$throw);
		}
	},
	function(env, after) {
...
/**
 * Get a user's most recent signup only
 */
var getRecentSignup = new fl.Chain(
	function(env, after, user) {
		if (null === user) {
			env.$throw(new Error('Invalid user given to getRecentSignup()'));
			return;
		}
...
/**
 * Retrieve users from the steam API and insert them into the database. Because the
 * steam API for obtaining profile information does not match the format of the
 * oauth response, we cannot use the same code path and have to reformat the results
 * before calling add user
 * @param[in] steamids Array of steam32 ids to be resolved and added
 */
let createUsers = new fl.Chain(
...
module.exports = {
	...
	search : search,
	getRecentSignup : getRecentSignup,
