...
// State for prelobby lib
var _self = {
	io : null
};

/**
 * Initialize a websocket pointer so that we can send client notifications of
 * events that happen on the backend
 * @param[in] io socket.io instance of the appropriate room
 */
function init(io) {
	_self.io = io;
...
/**
 * Send an event to the socket.io for this prelobby, if it exists
 */
function sendEvent(id, name, data) {
	if (_self.io) {
		_self.io.emit(name, data);
	}
}
...
var KBUpdate = new fl.Chain(
	...
	flHelper.IfTrue('results',
		function(env, after) {
			after(env.req.body.match, lobbies.RESULTS_FORMAT_KAEDEBOT, env.id);
		},
