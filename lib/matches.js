 ...
 * }
 */
function getPlayoffTemplate(teams) {
	...
	teams = Math.pow(2, N);
	var halfT = teams/2;
	bracket = getPlayoffTemplate(halfT);

	var start = halfT/2 - 1;
	var stop = halfT - 1;
	// Look for the game that is being expanded
	for (var j = start; j < stop; ++j) {

		bracket[j].home_previous = bracket.length + 1;
		bracket.push({
			playoff : bracket.length + 1,
		...
		bracket[j].away_previous = bracket.length + 1;
		bracket.push({
			playoff : bracket.length + 1,
