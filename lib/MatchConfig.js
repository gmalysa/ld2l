 ...
 * @param[in] io socket.io room for communicating with clients
 ...
 * @param[in] queue InhouseQueue object to reset when configuration is done
 * @todo timeouts
 * @todo button to cancel match
 */
function MatchConfig(io, players, id, queue) {
	this.io = io;
	this.players = _.map(players, lobbies.clientPlayerInfo);
	this.id = id;
	this.queue = queue;
	this.pickNumber = 0;
 ...
 */
MatchConfig.prototype.syncState = function() {
	this.io.emit('configStart', {
