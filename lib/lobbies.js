 ...
 * @param[in] swap If true, swap sides (for when rad/dire doesn't match home/away)
 * @return id The ld2l match id assigned to this game
 */
var parseResults = new fl.Chain(
	function(env, after, lobby, format, matchid, swap) {
		...
		if (RESULTS_FORMAT_KAEDEBOT == format) {
			var dire_win = (lobby.match.matchOutcome == "k_EMatchOutcome_DireVictory");
			...
			match.result = (lobby.radiant_win != swap) ? matches.HOME_WIN : matches.AWAY_WIN;
		...
		if (matchid > 0) {
			env.filters.matches.select({id : matchid}).exec(after, env.$throw);
		}
		else {
