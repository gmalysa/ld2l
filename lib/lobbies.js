 ...
 * side-safe one already, it produces a copy. Not all fields need to be present
 * @param[in] player The player object to save client-safe fields for
 * @return Copy of player with only data that we like to send to a client
 */
...
var isSwapNeeded = new fl.Chain(
	...
	function(env, after, match) {
		...
		// If this was an inhouse (home/away = 0) never swap
		if (undefined === match.home.id || undefined === match.away.id)
			env.result = false;
 ...
 * @param[in] matchid ld2l ID for the match to be updated, must be set
 */
var parseResults = new fl.Chain(
	function(env, after, lobby, format, matchid) {
		...
		if (!(matchid > 0)) {
			env.$throw(new Error('Match ID must be positive'));
			return;
		}
	...
	function(env, after, swap) {
		...
		if (RESULTS_FORMAT_KAEDEBOT == env.format) {
			...
			env.dotaid = env.lobby.match.matchId;
			env.result = result;
			...
			env.result = (env.lobby.radiant_win != env.swap) ? matches.HOME_WIN : matches.AWAY_WIN;
			env.dotaid = env.lobby.match_id;
			env.match = env.lobby;
	...
	function(env, after, results) {
		...
		if (results.length > 0 && results[0].dotaid > 0) {
			env.$throw(new Error('Match '+env.dotaid+' has already been recorded'));
			return;
		}
