...
/**
 * Vouch a player as another player. This requires that env.user has the PRIVS_VOUCH priv
 * @param[in] env.user The user doing the vouching
 * @param[in] steamid The steam ID of the user we're vouching
 * @todo audit log, add who vouched someone to the db
 */
var vouchPlayer = new fl.Branch(
	isLoggedIn,
	new fl.Chain(
		_pushSteamID,
		getPrivs,
		function(env, after, userPrivs) {
			if (!_hasPriv(userPrivs, PRIVS_VOUCH)) {
		...
		function(env, after, user) {
			if (user === null) {
				env.$throw(new Error('You can\'t vouch someone who hasn\'t registered'));
			}
			else {
				env.filters.privs.insert({steamid : user.steamid, priv : PRIVS_JOIN_SEASON})
					.exec(after, env.$throw);
			}
		}
	),
